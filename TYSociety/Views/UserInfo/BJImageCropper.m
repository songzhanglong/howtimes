...
//    rightView = [self newEdgeView];
    ...
    topView = [self newCornerView];
    bottomView = [self newCornerView];
    leftView = [self newCornerView];
    rightView = [self newCornerView];
...
-(void)setProportion:(float)proportion{
    ...
    if (proportion!=0.) {
        CGRect r=cropView.frame;
        r.size.height=r.size.width/proportion;
        cropView.frame=r;
    }
    [self updateBounds];
...
- (CGRect)calcFrameWithImage:(UIImage*)image andMaxSize:(CGSize)maxSize {
    ...
    CGRect noScale = CGRectMake((maxSize.width-(image.size.width * imageScale + increase))/2, (maxSize.height-(image.size.height * imageScale + increase))/2, image.size.width + increase, image.size.height + increase);
    ...
    scaled = CGRectMake((maxSize.width-(image.size.width * imageScale + increase))/2, (maxSize.height-(image.size.height * imageScale + increase))/2, image.size.width * imageScale + increase, image.size.height * imageScale + increase);
...
//                frame.size.height=frame.size.width/self.proportion;
//            }
//            
//            cropView.frame = frame;
//            
            ...
            if (isPanning) {
                ...
                if (scaleDistance != 0) {
                    ...
                    if (newSize.width >= 50 && newSize.height >= 50 && newSize.width <= CGWidth(cropView.superview.frame) && newSize.height <= CGHeight(cropView.superview.frame)) {
                        if (self.proportion!=0.) {
                            newSize.height=newSize.width/self.proportion;
                        }
                ...
                {
                    if (self.proportion!=0.) {
                        height=CGWidth(cropView.frame)/self.proportion;
                    }
                ...
                if (width > 30 || cropView.frame.size.width < 45)
                {
                    ...
                    cropView.frame = CGRectMake(x, CGOriginY(cropView.frame), width, self.proportion!=0.?width/self.proportion:CGHeight(cropView.frame));
                }
            }
        } break;
    }
