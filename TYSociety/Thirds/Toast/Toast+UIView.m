...
- (void)makeToastActivityToMsg:(NSString *)message
{
    UIView *existingActivityView = (UIView *)objc_getAssociatedObject(self, &CSToastActivityViewKey);
    if (existingActivityView != nil) {
        UILabel *label = (UILabel *)[existingActivityView viewWithTag:1];
        [label setText:message];
        return;
    }
    
    UIView *activityView = [[[UIView alloc] initWithFrame:CGRectMake(0, 0, CSToastActivityWidth + 20, CSToastActivityHeight)] autorelease];
    ...
    UILabel *label = [[[UILabel alloc] initWithFrame:CGRectMake(0, activityIndicatorView.frameBottom + 5, activityView.frameWidth, 20)] autorelease];
