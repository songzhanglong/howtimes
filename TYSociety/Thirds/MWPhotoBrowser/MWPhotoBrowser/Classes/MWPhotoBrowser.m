...
@interface MWPhotoBrowser ()<AVAudioPlayerDelegate,UICollectionViewDataSource,UICollectionViewDelegate>
...
- (UIInterfaceOrientationMask)supportedInterfaceOrientations {
    return _collectionView ? UIInterfaceOrientationMaskPortrait : UIInterfaceOrientationMaskAll;
}
...
- (void)beginMakeGrow:(id)sender
{
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(shouldEditTouch:and:)]) {
        canMake = [_delegate shouldEditTouch:_currentPageIndex and:self];
    }
...
- (void)playVoice:(NSString *)urlStr
{
    ...
    [HttpClient downloadFileWithProgress:urlStr complateBlcok:^(NSError *error, NSURL *filePath) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [weakSelf downFinish:error ? NO : YES Data:nil file:[filePath absoluteString]];
        });
