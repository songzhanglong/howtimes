...
#define kRZPopupMenuCenterBtnCenterX  ([UIScreen mainScreen].bounds.size.width / 2)
#define kRZPopupMenuCenterBtnCenterY ([UIScreen mainScreen].bounds.size.height - 49 + 55.0 / 6)
...
CGFloat const kRZPopupMenuItemWholeAngle = M_PI * 0.8;
CGFloat const kRZPopupMenuItemStartAngle = M_PI * 1.1;
CGFloat const kRZPopupMenuItemExpandRotation = M_PI;
CGFloat const kRZPopupMenuItemCloseRotation = M_2_PI;
CGFloat const kRZPopupMenuCenterBtnWidth = 55.0f;
...
- (void)setupMenuItems
{
    ...
    for (RZPopupMenuItem *item in self.menuItems) {
        ...
        item.width = kRZPopupMenuCenterBtnWidth - 10;
        item.height = kRZPopupMenuCenterBtnWidth - 10;
...
{
    if (!_centerBtn) {
        _centerBtn = [[RZPopupMenuItem alloc] initWithImage:CREATE_IMG(@"homeAddHlight") highlightedImage:nil contentImage:nil contentHighlightedImage:nil];
...
- (NSMutableArray *)menuItems
{
    if (!_menuItems) {
        ...
        NSArray *imgs = @[@"homeTakePhoto",@"homeTakeVideo",@"homeMail",@"homeText",@"homeVoice"];
        for (int i = 0; i < imgs.count; i ++) {
            ...
            RZPopupMenuItem *item = [[RZPopupMenuItem alloc] initWithImage:CREATE_IMG(imgs[i])  highlightedImage:nil contentImage:nil contentHighlightedImage:nil];
            item.delegate = self;
...
- (void)setExpanded:(BOOL)expand
{
    ...
    CGFloat angle = /*expand ? - M_PI : */0.0f;
    CAKeyframeAnimation *rotationAni = [CAKeyframeAnimation animationWithKeyPath:@"transform.rotation.z"];
